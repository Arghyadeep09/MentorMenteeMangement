...
const MentorDashboard = () => {
    ...
    const fetchMentorDetails = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://localhost:5000/api/mentor/details?mentorId=${user.id}`,
  ...
  useEffect(() => {
    const fetchAvailability = async () => {
      ...
      try {
        const token = await auth.currentUser.getIdToken();
        const response = await axios.get(
          `http://localhost:5000/api/mentor/slots?mentorId=${user.id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const fetchBookings = async () => {
    try {
      const token = await auth.currentUser.getIdToken();
      const response = await axios.get(
        `http://localhost:5000/api/mentor/bookings?mentorId=${user.id}`,
        {
          headers: {
  ...
  const toggleAvailability = async (day, time) => {
    ...
    try {
      ...
      if (updatedAvailability[day]) {
        ...
        await axios.delete(
          `http://localhost:5000/api/mentor/delete-slot?mentorId=${user.id}&day=${day}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      ...
      await axios.post(
        "http://localhost:5000/api/mentor/create-slot",
  ...
  const updateMentorDetails = async () => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/api/mentor/update?mentorId=${user.id}`,
        editableDetails,
        {
          headers: {
            Authorization: `Bearer ${token}`,
